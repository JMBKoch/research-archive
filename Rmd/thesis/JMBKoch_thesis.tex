...
cross-loadings to zero, while not shrinking (relevant) large cross-loadings to avoid bias. However, the big caveat of this prior is that it cannot be implemented in STAN (Stan Development Team, 2021), the most advisable MCMC-package for complex, highly-dimensional Bayesian models, due to being a discrete mixture prior (Betancourt, 2018; Stan Development Team, 2021). This calls for non-discrete alternative shrinkage-prior
...
When plotting the density of \(k_{jk}\) there is a very high peak at low values around\ldots{} and a very high peak of high values, resulting in a plot that resembles a horseshoe. This illustrates that the horseshoe prior possesses the desired property of either shrinking parameters very little, or very much, with very few parameters that are shrunken in a non-extreme fashion.

While the Horseshoe Prior shows to possess these desired theoretical properties in practice (Carvalho et al., 2010; Datta \& Ghosh, 2013; Polson \& Scott, 2010; Van Der Pas, Kleijn, \& Van Der Vaart, 2014), not shrinking large parameters at all can lead to identification issues (see Ghosh, Li, \& Mitra, 2018). The RHSP overcomes this by shrinking also large parameters a little bit, as it is designed such that for large parameters the prior approaches a normal (slab) prior with mean zero and variance \(c^2\) (Piironen \& Vehtari, 2017b).
