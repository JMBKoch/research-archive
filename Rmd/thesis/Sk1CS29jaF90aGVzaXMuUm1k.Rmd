...
The top left panel of Figure 4 summarizes the Power (true-positive rate) in selecting the truly non-zero cross-loadings as non-zero of the SVNP, per set of conditions and selection criterion. Again, the power of the first item in the set of truly non-zero cross-loading (i.e., $\lambda_{c,1}$ is presented representative for the two truly non-zero crossloadings. The horizontal red dash line indicates the minimum power of .80 recommended by @muthen_bayesian_2012.  
