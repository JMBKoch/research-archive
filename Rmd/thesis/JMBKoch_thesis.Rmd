...
$$Y = \mathbf{\beta} X + \mathbf{e}$$, where 
$$$$, 
...
Penalized regression adds a a penality term to this cost function 

When ... ridge, when.. lasso. 

Hier ook alvast ingaan op @jacobucci_regularized_2016


In a Bayesian context, instead of adding a penalty to the cost function of a model, so-called shrinkage-priors are set for the parameters [@van_erp_shrinkage_2019]. I

- Bayesian Estimatation: Joint Posterior ~ Prior x Data
- Thereby prior set for parameter allows to . 
- Simple case: setting normal prior for regression coefficients, which corresponds. 
- RESEMBLESthen lasso (...) > 

The well-known ridge- [@hoerl_ridge_2000] and lasso-penalization [@tibshirani_regression_1996] in regression resembles to setting a ridge-prior [@hsiang_bayesian_1975] or a Laplace-prior [@park_bayesian_2008] for regression coefficients respectively.
...
regularized Bayesian SEM is the so-called Spike-and-Slab Prior [@george_variable_1993; @ishwaran_spike_2005; @mitchell_bayesian_1988]. This
...
A particularly promising candidate is the so-called *Regularized Horseshoe Prior* [RHSP, @piironen_hyperprior_2017; @piironen_sparsity_2017]. This prior is an extension of the Horseshoe Prior [@carvalho_horseshoe_2010]. The main idea of this prior is that there is a *global shrinkage parameter* $\tau$, shrinking all cross-loadings to zero, and a *local shrinkage parameter*  $\bar{\omega}_{jk}$^[We deviate from the common notation of the local shrinkage parameter as $\bar{\lambda}$, as this letter is commonly used to denote factor loadings in CFA.] that allows truly large cross-loadings to escape the shrinkage. Formally, the Horseshoe prior for every cross-loading of factor j on item k is specified as follows:
...
The name-giving intuition behind the horseshoe prior becomes clear when considering the finding that [@piironen_sparsity_2017]:
...
Pas, Kleijn and van der Vaart, 2014), not shrinking large parameters at all can lead to identification issues [see @ghosh_use_2018]. (HOEVEEL UITLEG IS HIER NOG NODIG?).

The RHSP overcomes this by shrinking also large parameters a little bit, as it is designed such that for large parameters the prior approaches a normal (slab) prior with mean zero and variance $c^2$ [@piironen_sparsity_2017]. 
