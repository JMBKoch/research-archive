...
cross-loadings to zero, while not shrinking (relevant) large cross-loadings to avoid bias.  However, the big caveat of this prior is that it cannot be implemented in STAN [@stan_development_team_stan_2021], the most advisable MCMC-package for complex, highly-dimensional Bayesian models, due to being a discrete mixture prior (Betancourt, 2018; Stan Development Team, 2021). This calls for non-discrete alternative shrinkage-prior
